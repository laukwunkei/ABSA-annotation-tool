...
def exit():
    ...
    with open(output_path, 'r') as myfile:
        data=myfile.read()
    data = json.loads(data)
    num_annotation = len(data)
    num_unsentiment,num_neutral,num_pos,num_negative = summary(data)
    return render_template('success.html', num_annotation=num_annotation, num_unsentiment=num_unsentiment,num_neutral=num_neutral,num_pos=num_pos,num_negative=num_negative)
